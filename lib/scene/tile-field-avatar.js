...
TileFieldAvatar.prototype.draw = function (context) {
    if (this._altered) {
        ...
        if (!this._canv.width || !this._canv.height) return;

        this._drawOperations.sort(function (a, b) {
    ...
    } else if (!this._canv.width || !this._canv.height) return;

    context.drawImage(this._canv, 0, 0, this._canv.width, this._canv.height, this._x, this._y, this._w, this._h);
};

TileFieldAvatar.prototype._recordTile = function (type, position) {
    if (!this._type2tiles[type]) this._type2tiles[type] = {};
    this._type2tiles[type][position] = true;
};
TileFieldAvatar.prototype._derecordTile = function (type, position) {
    if (this._type2tiles[type]) delete this._type2tiles[type][position];
};
...
TileFieldAvatar.prototype._excribeIcon = function (icon, x, y, w, h) {
    if (icon.atlas) this._excribe.apply(this, icon.bounds(x, y, w, h));
};
...
TileFieldAvatar.prototype._drawIcon = function (icon, x, y, w, h) {
    if (sanity.throws) {
        if (sanity(is.number(x)) || sanity(is.number(y)) || sanity(is.number(w)) || sanity(is.number(h))) return;
    }
    if (!icon || !icon.atlas) return;
    this._addAtlas(icon.atlas);
    if (this._isLoaded) icon.draw(this._context, x, y, w, h);
};
...
TileFieldAvatar.prototype.drawSelectedTile = function (selector, type, x, y, layer, observedTypes) {
    ...
    if (sanity(is.defined(layer))) layer = 0;
    if (!observedTypes) observedTypes = [type];
    ...
    function hasTilei(dx, dy) {
        ...
        return observedTypes.some(function (t) {
            return _this.fieldGroup.hasTile(dest, t);
        });
    }

    function select() {
        return selector(hasTilei(-1, -1), hasTilei(0, -1), hasTilei(1, -1), hasTilei(-1, 0), hasTilei(1, 0), hasTilei(-1, 1), hasTilei(0, 1), hasTilei(1, 1));
    }
    ...
    var op = {
        ...
        execute: function execute() {
            select().forEach(function (icon, index) {
                if (!icon) return;
                _this._drawIcon(icon, Math.floor((x + xs[index]) * _this._tileSize - _this._minX), Math.floor((y + ys[index]) * _this._tileSize - _this._minY), _this._tileSize / 2, _this._tileSize / 2);
            });
        },
        excribe: function excribe() {
            select().forEach(function (icon, index) {
                if (!icon) return;
                _this._excribeIcon(icon, (x + xs[index]) * _this._tileSize, (y + ys[index]) * _this._tileSize, _this._tileSize / 2, _this._tileSize / 2);
            });
        }
    };
...
TileFieldAvatar.prototype.drawTile = function (icon, x, y, layer, type) {
    ...
    if (sanity(icon)) return function () {};
    if (sanity(is.defined(layer))) layer = 0;
    ...
    var op = {
        ...
        layer: y + layer,
        execute: function execute() {
            _this._drawIcon(icon, Math.floor(x * _this._tileSize - _this._minX), Math.floor(y * _this._tileSize - _this._minY), _this._tileSize, _this._tileSize);
        },
        excribe: function excribe() {
    ...
    return function () {
        if (type) _this._derecordTile(type, here);
        removeFromArray(_this._drawOperations, op);
        _this._alter();
    };
};
...
//# sourceMappingURL=tile-field-avatar.js.map