...
ObjectPool.prototype.make = function () /*varargs*/{
    ...
    if (this.deadCount) {
        ...
        if (sanity.throws) this.dead[this.deadCount] = undefined;
    } else {
...
ObjectPool.prototype.remove = function (removals) {
    ...
    for (var i = 0; i < initialCount; i += 1) {
        ...
        var deleting = false;
        if (shiftBy < removals.length) for (var j = 0; j < removals.length; j += 1) {
            if (removals[j] === obj) {
                deleting = true;
                break;
            }
        ...
        if (deleting) {
            ...
            this.deadCount += 1;
            shiftBy += 1;
        } else if (shiftBy && i - shiftBy >= 0) this.alive[i - shiftBy] = obj;
    }
